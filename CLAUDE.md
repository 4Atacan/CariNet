...
- [ ] **DAĞITIM ELLE (gerileme):** imaj derlemesi CI'da üç kez 30 dk zaman aşımına düştü; aynı derleme yerelde sorunsuz. İmajlar yerelde derlenip `docker save | ssh docker load` ile gönderiliyor, compose `carinet-*:local` kullanıyor. `cache-to` `mode=min`'e çekildi — CI düzelirse compose **ghcr'ye geri alınmalı**
