...
- [ ] **Yedeğin iki eksiği:** (1) age **özel anahtarı hâlâ yalnız db sunucusunda** — sunucu giderse yedekler çözülemez, parola yöneticisine alınmalı; (2) **gerçek dış konum yok** — kopya aynı sağlayıcının ikinci makinesinde, R2 için Cloudflare hesabı bekleniyor
...
- [ ] **ONBOARDING KİLİDİ (pilot öncesi kapanmalı):** prod'da yeni SELLER_ADMIN **kendi başına giriş yapamaz** — kural #11 gereği 2FA'sız giriş engelli, ama `/auth/2fa/setup` **giriş yapmış** olmayı istiyor. İlk hesap elle tohumlandı (2FA hazır oluşturuldu); her yeni satıcı için bu el işi tekrarlanamaz. Kalıcı çözüm: davet/ilk-giriş akışı (tek kullanımlık token → parola belirle + 2FA kur)
- [ ] **Parola değiştirme yok** — kodda böyle bir uç yok; kullanıcı kendi parolasını değiştiremiyor (19.07'de sunucu betiğiyle yapıldı, ürün davranışı olamaz)
