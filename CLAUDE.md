...
- [ ] **2FA kurulumunda QR kodu yok** — kurulum anahtarı elle giriliyor. Engel kod değil ORTAM: **depo OneDrive altında** (`C:\Users\Ayurd\OneDrive\...`) ve OneDrive senkronizasyonu pnpm'in atomik dosya taşımalarını kesiyor → `pnpm add` her denemede `ERR_PNPM_ENOENT ... <paket>_tmp_NNNN` ile düşüyor. **Pakete özgü değil** (önce `archiver-utils`, sonra `qrcode` üzerinde patladı) — yani BU MAKİNEDE yeni bağımlılık eklenemiyor. Docker içindeki kurulumlar sorunsuz; hata yalnız Windows/OneDrive tarafında.
      **Kalıcı çözüm:** depoyu OneDrive dışına taşı (ör. `C:\src\CariNet`). Bu yapılmadan QR ve başka her yeni paket bloke.
