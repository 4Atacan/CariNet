...
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| `sellers`                    | name, slug UNIQUE, logo_url, is_active                                                                                                                                                        | Tenant kökü; slug misafir ödeme URL'inde                            |
...
Hata kodları `packages/shared/errors.ts`: VALIDATION_ERROR · UNAUTHORIZED · TENANT_FORBIDDEN · NOT_FOUND · CREDIT_LIMIT_EXCEEDED · INTENT_EXPIRED · ALREADY_CONFIRMED · IMPORT_ROW_ERRORS…
...
Uç grupları: `/auth` (login, refresh, logout, forgot, switch-account, sessions, revoke-all) · `/buyers` · `/transactions` · `/invoices` · `/reports` (risk, periodic-balance, average-due) · `/collections` (intent, guest/{sellerSlug}, statement-import, confirm, pos-callback) · `/imports` (wizard, ubl, presets) · `/products` · `/campaigns` · `/notifications` · `/exchange-rates` · `/audit`.
...
- [ ] `CollectionProvider` + `BankTransferProvider` · intent yaşam döngüsü + expiry cron
- [ ] Mobil "Ödeme Yap" (referans+IBAN, kopyala/paylaş) · Panel: bekleyen intent + manuel onay + **ekstre importu→eşleştirme→toplu onay**
- [ ] Kısmi/açıklamasız/iade senaryoları
- [ ] `seller_pos_configs` CRUD (AES, 2FA'lı değişiklik) · ilk POS adaptörü (pilotun sağlayıcısı) hosted 3D + callback imza+idempotency · Taksit Seçenekleri ekranı
- [ ] Ortak hat: CONFIRMED→CREDIT+push+audit · misafir `pay/{sellerSlug}` kanal seçimli
      **✅ Bitti:** (1) sahte ekstreyle havale akışı uçtan uca; (2) sandbox kartla hosted sayfadan ödeme→callback→bakiye düşer; (3) aynı intent/satır/callback ikinci kez işlenemiyor — üçü test kanıtlı.
